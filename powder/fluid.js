class Fluid{
    constructor(size, iter, diff, visc, dt){
        ...
        this.obstacles = new Array(this.N * this.N).fill(0);
    }
    ...
    set_obstacle(x, y) {
        this.obstacles[this.idx(x, y)] = 1;
    }

    clear_obstacle(x, y) {
        this.obstacles[this.idx(x, y)] = 0;
    }
    ...
    set_boundary(b, x){
        ...
        for (let i = 1; i < N - 1; i++) {
            for (let j = 1; j < N - 1; j++) {
                if (this.obstacles[this.idx(i, j)] === 1) {
                    // sides
                    if (this.obstacles[this.idx(i - 1, j)] === 0) {
                        x[this.idx(i, j)] = b === 1 ? -x[this.idx(i - 1, j)] : x[this.idx(i - 1, j)];
                    }
                    if (this.obstacles[this.idx(i + 1, j)] === 0) {
                        x[this.idx(i, j)] = b === 1 ? -x[this.idx(i + 1, j)] : x[this.idx(i + 1, j)];
                    }
                    if (this.obstacles[this.idx(i, j - 1)] === 0) {
                        x[this.idx(i, j)] = b === 2 ? -x[this.idx(i, j - 1)] : x[this.idx(i, j - 1)];
                    }
                    if (this.obstacles[this.idx(i, j + 1)] === 0) {
                        x[this.idx(i, j)] = b === 2 ? -x[this.idx(i, j + 1)] : x[this.idx(i, j + 1)];
                    }
...
for (let i = 38; i < 58; i++) {
    for (let j = 38; j < 58; j++) {
        fluid.set_obstacle(i, j);
    }
}
fluid.step();

function triggerExplosion(fluid, centerX, centerY, explosionDensity, explosionForce) {
    const N = fluid.N;

    // Determine explosion radius
    const radius = Math.floor(N * 0.05); // Explosion affects 10% of the grid

    for (let j = Math.max(0, centerY - radius); j < Math.min(N, centerY + radius); j++) {
        for (let i = Math.max(0, centerX - radius); i < Math.min(N, centerX + radius); i++) {
            const dx = i - centerX;
            const dy = j - centerY;
            ...
            if (distance <= radius) {
                // Add high density
                const densityFactor = (1 - distance / radius) * explosionDensity;
                fluid.add_density(i, j, densityFactor);

                // Add velocity outward from the explosion center
                const forceFactor = explosionForce * (1 - distance / radius);
                const velocityX = forceFactor * (dx / distance || 0); // Avoid division by zero
                const velocityY = forceFactor * (dy / distance || 0);
                fluid.add_velocity(i, j, velocityX, velocityY);
            }
        }
    }
}

triggerExplosion(fluid, 30, 30, 5000, 100);

let cellSize = width / N;

function draw(){
    console.log(Math.max(...fluid.density));
    ctx.fillStyle = "#000000";
    ctx.fillRect(0, 0, width, height);
    for(let j = 1; j < N-1; j++){
        for(let i = 1; i < N-1; i++){
            if(fluid.get_obstacle(i, j)) continue;
...
function get_color(d){
    let t = d >= 10.0 ? 10.0 : d <= 0.0 ? 0.0 : d;
    return `hsl(${255-t*25.5}, 100%, 50%)`;
...
function mouse_move(e){
    if(drag_button < 0) {
        ...
        let x = pos.x;
        let y = pos.y;
        let i = Math.floor(pos.x / cellSize);
        let j = Math.floor(pos.y / cellSize);
        fluid.add_density(i, j, 100 * (drag_button == 0 ? 1 : -1));
        fluid.add_velocity(i, j, 2*(x - drag_x), 2*(y - drag_y));
        drag_x = x;
        drag_y = y;
    }
    else {
